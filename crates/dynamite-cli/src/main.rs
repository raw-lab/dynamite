...
use dynamite_core::fasta::{self, Record};
use dynamite_core::gencode::GeneticCode;
use dynamite_core::gene::Format;
use dynamite_core::model::{CodeChoice, EngineKind, Mode};
use dynamite_core::annotate::{annotate, AnnotateConfig};
use dynamite_core::{codon, doctor, rna, run_records, sixframe, RunConfig};
...
fn cmd_call(a: CallArgs) -> Result<()> {
    ...
    dynamite_core::gene::write_all(&mut buf, &out.genes, format, &out.contigs);
...
fn cmd_all(a: AllArgs) -> Result<()> {
    ...
    use dynamite_core::gene::{write_all, write_genbank_full};
...
fn cmd_translate(a: TranslateArgs) -> Result<()> {
    ...
    if a.orfs {
        ...
        for r in &records {
            let s = dynamite_core::seq::Seq::from_ascii(&r.seq);
            genes.extend(sixframe::orfs(&r.id, &s, &code, a.minlen, a.require_start));
            contigs.push((r.id.clone(), r.seq.len()));
        }
        dynamite_core::gene::write_all(&mut buf, &genes, format, &contigs);
